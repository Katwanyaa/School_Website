...
const router = useRouter();
  ...
  const getCurrentFeeItems = () => {
    ...
    const buildFeeItem = (name, amount, description) => [{
      id: name.toLowerCase().replace(/\s+/g, '-'),
      name,
      amount: amount || 0,
      description: description || `${name} information from the school documents API.`,
      optional: false
    }];
    ...
    switch(activeTab) {
      case 'day':
        return buildFeeItem(
          'Day School Fees',
          documentData.feesDayAnnualAmount,
          documentData.feesDayDescription || 'Total annual fees for day school'
        );
      case 'boarding':
        return buildFeeItem(
          'Boarding School Fees',
          documentData.feesBoardingAnnualAmount,
      ...
      case 'admission':
        return buildFeeItem(
          'Admission Letter',
          0,
          documentData.admissionFeeDescription || 'Admission letter document uploaded via school documents API.'
        );
      default:
        return [];
    }
  ...
  const getCurrentTotal = () => {
    ...
    switch(activeTab) {
      case 'day':
        return documentData.feesDayAnnualAmount || 0;
      case 'boarding':
        return documentData.feesBoardingAnnualAmount || 0;
      case 'admission':
        return 0;
      default:
        return 0;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tighter leading-[1.1]">
                  School <span className="text-white">Fees</span>
                </h1>
    ...
    >
      ...
      <span className="text-xs sm:text-sm">Download</span>
            ...
            <p className="text-[10px] sm:text-xs md:text-sm text-slate-500">
              {documentData?.feesDayAnnualAmount ? 'Total annual fees' : 'Annual day school fees not set'}
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-4 sm:p-5 md:p-6 shadow-sm">
            ...
            <p className="text-[10px] sm:text-xs md:text-sm text-slate-500">
              {documentData?.feesBoardingAnnualAmount ? 'Total annual fees' : 'Annual boarding fees not set'}
            </p>
          </div>
